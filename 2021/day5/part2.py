...
def solution(lines):
    ...
    for l in lines:
        ...
        elif maxX - minX == maxY - minY: #diagonal
            slope = (p2[1] - p1[1]) // (p2[0] - p1[0])
            left_most_point = p1 if p1[0] == minX else p2
            for step in range(maxX - minX  + 1):
                x = left_most_point[0] + step
                y = left_most_point[1] + (step * slope)
                points_totals[point(x, y)] += 1
